...
void xxyDist::remove(const instance& i) {

	//remove instance from current distribution
	...
	for (CategoricalAttribute x1 = 1; x1 < getNoCatAtts(); x1++) {
		...
		for (CategoricalAttribute x2 = 0; x2 < x1; x2++) {
			...
			xySubDist.decCount(x2, v2, theClass);

			if (verbosity >= 3)
				if (xyCounts.count < 3) {
					printf("tell me !%d,%d,%d,%d,%d,%d,%d\n", x1, v1, x2, v2,
							*ref(x1, v1, x2, v2, theClass), xyCounts.count,
							getNoCatAtts());
...
void xxyDist::add(const instance& i) {

	//add instance to current distribution
	//the difference with update is that here xyCounts need not updating
