...
class kdbext2 : public IncrementalLearner {
public:
    kdbext2();
    kdbext2(char*const*& argv, char*const* end);
    ~kdbext2(void);
...
protected:
    ...
    //    std::vector<std::vector<distributionTree> > dTree_; // used in the second pass and for classification
    //    std::vector<std::vector<std::vector<CategoricalAttribute> > > parents_;
    std::vector<std::vector<distributionTree> > dTree_; // used in the second pass and for classification
    std::vector<std::vector<std::vector<CategoricalAttribute> > > parents_;
    std::vector<std::vector<std::vector<int > > > ModelOrder;
    ...
    std::vector<std::vector<std::vector<int> > > order_cross_all;
    float threshold;
    std::vector<float> sum_cmi;
    std::vector<double> posterior_all;
    std::vector<std::vector<double> > Wij;
    std::vector<unsigned int> seq;
    
    std::vector<float> Accuracy_different;
     int max_flag;
     xxyDist xxyDist_; ///< the xxy distribution that aode learns from the instance stream and uses for classification
     xxxyDist xxxyDist_;
     std::vector< std::vector < double > > posteriorDist_different;
    std::vector<bool> SubCMI_label;
     unsigned int v;
    float P_dist_all;
};
