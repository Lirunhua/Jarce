...
class hnb_waode: public IncrementalLearner {
public:
	/**
	 ...
	 */
	hnb_waode(char* const *& argv, char* const * end);

	virtual ~hnb_waode(void);
...
//	std::vector<CategoricalAttribute> selectedAtt;
	...
	xxyDist xxyDist_; ///< the xxy distribution that aode learns from the instance stream and uses for classification
        ...
        std::vector<float> wi;//保存属性和类变量之间的权值
        float sumOfwi;
};
