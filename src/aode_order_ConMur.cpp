...
aode_order_ConMur::aode_order_ConMur(char* const *& argv, char* const * end) {
    name_ = "aode_order_ConMur";
    UsedAttrRatio = 0;
    weighted = false;
    minCount = 100;
...
aode_order_ConMur::~aode_order_ConMur(void) {
}

void aode_order_ConMur::getCapabilities(capabilities &c) {
    c.setCatAtts(true); // only categorical attributes are supported at the moment
}

void aode_order_ConMur::reset(InstanceStream &is) {
    xxxyDist_.reset(is);
    trainingIsFinished_ = false;
    instanceStream_ = &is;
...
void aode_order_ConMur::initialisePass() {

}

void aode_order_ConMur::train(const instance &inst) {
    xxxyDist_.update(inst);
}

/// true iff no more passes are required. updated by finalisePass()

bool aode_order_ConMur::trainingIsFinished() {
    return trainingIsFinished_;
}

void aode_order_ConMur::classify(const instance &inst, std::vector<double> &classDist) {
...
void aode_order_ConMur::finalisePass() {
...
void aode_order_ConMur::nbClassify(const instance &inst, std::vector<double> &classDist,
        xyDist &xyDist_) {
