...
//hnb_aode构造函数
hnb_aode::hnb_aode(char* const *& argv, char* const * end) {
    name_ = "HNB_AODE";
    UsedAttrRatio = 0;
    weighted = false;
    minCount = 100;
...
//hnb_aode析构函数
hnb_aode::~hnb_aode(void) {
}

//不需要管，就是对属性的一种操作
void hnb_aode::getCapabilities(capabilities &c) {
    c.setCatAtts(true); // only categorical attributes are supported at the moment
}

void hnb_aode::reset(InstanceStream &is) {
...
void hnb_aode::initialisePass() {

}

void hnb_aode::train(const instance &inst) {
    //根据每一个实例训练得结构
...
bool hnb_aode::trainingIsFinished() {
...
//
int hnb_aode::getNextElement(std::vector<CategoricalAttribute> &order, CategoricalAttribute ca, unsigned int noSelected) {
 ...
 * 该函数每次只对一个实例进行分类
 */
void hnb_aode::classify(const instance &inst, std::vector<double> &classDist) {
...
//NB分类器
void hnb_aode::nbClassify(const instance &inst, std::vector<double> &classDist,
        xyDist &xyDist_) {
