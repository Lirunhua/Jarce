...
{
public:
  XValInstanceStream(InstanceStream *source, const unsigned int noOfFolds, const unsigned int seed = 0,const unsigned int flag=0);
  ...
  InstanceCount size();    
  void setdivide(unsigned int d);///< the number of instances in the stream.  This may require a pass through the stream to determine so should be used only if absolutely necessary.
 // void setFlag(int sflag);
  // cross validation specific methods
  void startSubstream(const unsigned int fold, const bool training,const bool building);      ///< start training or testing for a new fold
