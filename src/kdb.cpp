...
void kdb::finalisePass() {
    ...
    if (pass_ == 1 && k_ != 0) {
        ...
        for (unsigned int x1 = 0; x1 < noCatAtts_; x1++) {
            ...
            if (parents_[x1].size() == 1) {
                printf("%d->%d\n", parents_[x1][0], x1);
            }
            if (parents_[x1].size() == 2) {
                printf("%d->%d,%d->%d\n", parents_[x1][0], x1, parents_[x1][1], x1);
            }
...
void kdb::getNoCatAtts_(unsigned int &NoCatAtt) {
    //printf(" getNoCatAtts_ \n");
    NoCatAtt = noCatAtts_;
}

void kdb::getStructure_kdb(std::vector<std::vector<CategoricalAttribute> > &parents) {
