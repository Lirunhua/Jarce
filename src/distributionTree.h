...
public:
  ...
  // update the counts as usual except for the leaf, where the numeric value is stored (needed in kdb-condDisc2)
  void update(const instance &i, const CategoricalAttribute att, const std::vector<CategoricalAttribute> &parents, NumValue attValue);
  
  // update a group of values for a combination of parents at a time (needed in kdb-condDisc) 
...
//  void updateClassDistributionForDifferentK(std::vector<double> &classDist, const CategoricalAttribute a, const instance &i, unsigned int k);
  ...
  void updateClassDistributionloocv(std::vector<double> &classDist, const CategoricalAttribute a, const instance &i, unsigned int k);  
