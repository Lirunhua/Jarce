...
class kdbEager :  public IncrementalLearner
{
public:
  kdbEager();
  kdbEager(char*const*& argv, char*const* end);
  ~kdbEager(void);
...
protected:
  ...
  std::vector<distributionTreeEager> dTree_;                      // used in the second pass and for classification
  std::vector<std::vector<CategoricalAttribute> > parents_;
  InstanceStream* instanceStream_;
  std::vector<double> SUM_Wij;
